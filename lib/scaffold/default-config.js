...
/**
 * Will return the path and default squarecore-node configuration. It will search for the
 * configuration file in the "~/.squarecore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.squarecore');
  var defaultConfigFile = path.resolve(defaultPath, './squarecore-node.json');

  if (!fs.existsSync(defaultPath)) {
    mkdirp.sync(defaultPath);
  }

  var defaultServices = ['squared', 'web'];
      ...
      servicesConfig: {
        squared: {
          spawn: {
            ...
            exec: path.resolve(__dirname, defaultPath, './data/squared')
          }
        }
      }
    };
