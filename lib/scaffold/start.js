...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for squared is necessary for this upgrade with the "reindex=1" square.conf option. \n' +
                 'There are changes necessary in both square.conf and squarecore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/bitpay/squarecore-node/blob/squared/docs/upgrade.md \n');
        ...
        servicesConfig: {
          squared: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/squared')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed squarecore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './squarecore-node.json');
  ...
  node.on('ready', function() {
    log.info('Squarecore Node ready');
  });
...
function loadModule(req, service) {
  try {
    // first try in the built-in squarecore-node services directory
    ...
    if (servicePackage.squarecoreNode) {
      serviceModule = service.name + '/' + servicePackage.squarecoreNode;
    }
