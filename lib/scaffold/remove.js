...
/**
 * Will remove a service from squarecore-node.json
...
/**
 * Will remove the Node.js service and from the squarecore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The squarecore-node configuration directory
...
function remove(options, done) {
  ...
  var squarecoreConfigPath = path.resolve(configPath, 'squarecore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(squarecoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a squarecore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to squarecore-node.json
        removeConfig(squarecoreConfigPath, service, next);
      });
    }, done
  );
}
