...
var BASE_PACKAGE = {
  description: 'A full Square coin node built with Bitcore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    'squarecore-lib': '^' + squarecore.version,
    'squarecore-node': version
  }
};

/**
 * Will create a directory and square.conf file for Square.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createSquareDirectory(datadir, done) {
 ...
 * @param {String} options.datadir - The square database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/squarecore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * square configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the squarecore node configuration directory
 * @param {String} options.datadir - The path to the square datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the squarecore-node directory and configuration
    ...
    function(next) {
      // Setup the square directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createSquareDirectory(absDataDir, next);
      } else {
        next();
      }
    },
