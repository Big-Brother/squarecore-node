...
describe('#add', function() {
  ...
  before(function(done) {
    mkdirp(testDir + '/s0/s1', function(err) {
      ...
      fs.writeFile(
        testDir + '/s0/s1/squarecore-node.json',
  ...
  describe('will modify scaffold files', function() {
    ...
    it('will update squarecore-node.json services', function(done) {
      var callCount = 0;
      var oldPackage = {
        dependencies: {
          'squarecore-lib': '^v0.13.17',
          'squarecore-node': '^v0.2.0'
        }
      };
      var spawn = sinon.stub().returns({
        stdout: {
      ...
      addtest({
        ...
        var configPath = path.resolve(testDir, 's0/s1/squarecore-node.json');
