...
var readFileSync = sinon.stub().returns(fs.readFileSync(path.resolve(__dirname, '../data/square.conf')));
var SquareService = proxyquire('../../lib/services/squared', {
  fs: {
    readFileSync: readFileSync
  }
});
var defaultSquareConf = fs.readFileSync(path.resolve(__dirname, '../data/default.square.conf'), 'utf8');
...
describe('Square Service', function() {
  ...
  describe('@constructor', function() {
    ...
    it('will create an instance without `new`', function() {
      var squared = SquareService(baseConfig);
      should.exist(squared);
    });
    it('will init caches', function() {
      var squared = new SquareService(baseConfig);
      should.exist(squared.utxosCache);
      should.exist(squared.txidsCache);
      should.exist(squared.balanceCache);
      should.exist(squared.summaryCache);
      should.exist(squared.transactionDetailedCache);

      should.exist(squared.transactionCache);
      should.exist(squared.rawTransactionCache);
      should.exist(squared.blockCache);
      should.exist(squared.rawBlockCache);
      should.exist(squared.blockHeaderCache);
      should.exist(squared.zmqKnownTransactions);
      should.exist(squared.zmqKnownBlocks);
      should.exist(squared.lastTip);
      should.exist(squared.lastTipTimeout);
    });
    it('will init clients', function() {
      var squared = new SquareService(baseConfig);
      squared.nodes.should.deep.equal([]);
      squared.nodesIndex.should.equal(0);
      squared.nodes.push({client: sinon.stub()});
      should.exist(squared.client);
    });
    it('will set subscriptions', function() {
      var squared = new SquareService(baseConfig);
      squared.subscriptions.should.deep.equal({
        address: {},
        rawtransaction: [],
        hashblock: [],
        transactionlock: []
  ...
  describe('#_initDefaults', function() {
    it('will set transaction concurrency', function() {
      var squared = new SquareService(baseConfig);
      squared._initDefaults({transactionConcurrency: 10});
      squared.transactionConcurrency.should.equal(10);
      squared._initDefaults({});
      squared.transactionConcurrency.should.equal(5);
    });
  });

  describe('@dependencies', function() {
    it('will have no dependencies', function() {
      SquareService.dependencies.should.deep.equal([]);
    });
  });
  ...
  describe('#getAPIMethods', function() {
    it('will return spec', function() {
      var squared = new SquareService(baseConfig);
      var methods = squared.getAPIMethods();
  ...
  describe('#getPublishEvents', function() {
    it('will return spec', function() {
      var squared = new SquareService(baseConfig);
      var events = squared.getPublishEvents();
      should.exist(events);
      events.length.should.equal(4);
      events[0].name.should.equal('squared/rawtransaction');
      events[0].scope.should.equal(squared);
      events[0].subscribe.should.be.a('function');
      events[0].unsubscribe.should.be.a('function');
      events[1].name.should.equal('squared/transactionlock');
      events[1].scope.should.equal(squared);
      events[1].subscribe.should.be.a('function');
      events[1].unsubscribe.should.be.a('function');
      events[2].name.should.equal('squared/hashblock');
      events[2].scope.should.equal(squared);
      events[2].subscribe.should.be.a('function');
      events[2].unsubscribe.should.be.a('function');
      events[3].name.should.equal('squared/addresstxid');
      events[3].scope.should.equal(squared);
    ...
    it('will call subscribe/unsubscribe with correct args', function() {
      var squared = new SquareService(baseConfig);
      squared.subscribe = sinon.stub();
      squared.unsubscribe = sinon.stub();
      var events = squared.getPublishEvents();

      events[0].subscribe('test');
      squared.subscribe.args[0][0].should.equal('rawtransaction');
      squared.subscribe.args[0][1].should.equal('test');

      events[0].unsubscribe('test');
      squared.unsubscribe.args[0][0].should.equal('rawtransaction');
      squared.unsubscribe.args[0][1].should.equal('test');

      events[1].subscribe('test');
      squared.subscribe.args[1][0].should.equal('transactionlock');
      squared.subscribe.args[1][1].should.equal('test');

      events[1].unsubscribe('test');
      squared.unsubscribe.args[1][0].should.equal('transactionlock');
      squared.unsubscribe.args[1][1].should.equal('test');

      events[2].subscribe('test');
      squared.subscribe.args[2][0].should.equal('hashblock');
      squared.subscribe.args[2][1].should.equal('test');

      events[2].unsubscribe('test');
      squared.unsubscribe.args[2][0].should.equal('hashblock');
      squared.unsubscribe.args[2][1].should.equal('test');
    });
  });
  ...
  describe('#subscribe', function() {
    ...
    it('will push to subscriptions', function() {
      var squared = new SquareService(baseConfig);
      var emitter = {};
      squared.subscribe('hashblock', emitter);
      squared.subscriptions.hashblock[0].should.equal(emitter);

      var emitter2 = {};
      squared.subscribe('rawtransaction', emitter2);
      squared.subscriptions.rawtransaction[0].should.equal(emitter2);
    });
  });
  ...
  describe('#unsubscribe', function() {
    ...
    it('will remove item from subscriptions', function() {
      ...
      var emitter4 = {};
      var emitter5 = {};
      squared.subscribe('hashblock', emitter1);
      squared.subscribe('hashblock', emitter2);
      squared.subscribe('hashblock', emitter3);
      squared.subscribe('hashblock', emitter4);
      squared.subscribe('hashblock', emitter5);
      squared.subscriptions.hashblock.length.should.equal(5);

      squared.unsubscribe('hashblock', emitter3);
      squared.subscriptions.hashblock.length.should.equal(4);
      squared.subscriptions.hashblock[0].should.equal(emitter1);
      squared.subscriptions.hashblock[1].should.equal(emitter2);
      squared.subscriptions.hashblock[2].should.equal(emitter4);
      squared.subscriptions.hashblock[3].should.equal(emitter5);
    });
    it('will not remove item an already unsubscribed item', function() {
      var squared = new SquareService(baseConfig);
      var emitter1 = {};
      var emitter3 = {};
      squared.subscriptions.hashblock= [emitter1];
      squared.unsubscribe('hashblock', emitter3);
      squared.subscriptions.hashblock.length.should.equal(1);
      squared.subscriptions.hashblock[0].should.equal(emitter1);
    });
  });
  ...
  describe('#subscribeAddress', function() {
    ...
    it('will not an invalid address', function() {
      ...
      squared.subscribeAddress(emitter, ['invalidaddress']);
      should.not.exist(squared.subscriptions.address['invalidaddress']);
    });
    it('will add a valid address', function() {
      var squared = new SquareService(baseConfig);
      var emitter = new EventEmitter();
      squared.subscribeAddress(emitter, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
    });
    it('will handle multiple address subscribers', function() {
      var squared = new SquareService(baseConfig);
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      squared.subscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      squared.subscribeAddress(emitter2, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
    });
    it('will not add the same emitter twice', function() {
      ...
      squared.subscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.exist(squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddress', function() {
    ...
    it('it will remove a subscription', function() {
      ...
      should.exist(squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
      squared.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
    it('will unsubscribe subscriptions for an emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      squared.unsubscribeAddress(emitter1);
      squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
    });
    it('will NOT unsubscribe subscription with missing address', function() {
      ...
      var emitter2 = new EventEmitter();
      squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      squared.unsubscribeAddress(emitter1, ['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs']);
      squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(2);
    });
    it('will NOT unsubscribe subscription with missing emitter', function() {
      ...
      var emitter1 = new EventEmitter();
      var emitter2 = new EventEmitter();
      squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter2];
      squared.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'][0].should.equal(emitter2);
    });
    it('will remove empty addresses', function() {
      ...
      squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      squared.unsubscribeAddress(emitter1, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      squared.unsubscribeAddress(emitter2, ['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
      should.not.exist(squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi']);
    });
    it('will unsubscribe emitter for all addresses', function() {
      ...
      var emitter2 = new EventEmitter();
      squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      squared.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'] = [emitter1, emitter2];
      sinon.spy(squared, 'unsubscribeAddressAll');
      squared.unsubscribeAddress(emitter1);
      squared.unsubscribeAddressAll.callCount.should.equal(1);
      squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      squared.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'].length.should.equal(1);
    });
  });
  ...
  describe('#unsubscribeAddressAll', function() {
    ...
    it('will unsubscribe emitter for all addresses', function() {
      ...
      squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'] = [emitter1, emitter2];
      squared.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'] = [emitter1, emitter2];
      squared.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'] = [emitter2];
      squared.subscriptions.address['7d5169eBcGHF4BYC6DTffTyeCpWbrZnNgz'] = [emitter1];
      squared.unsubscribeAddress(emitter1);
      squared.subscriptions.address['8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi'].length.should.equal(1);
      squared.subscriptions.address['XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs'].length.should.equal(1);
      squared.subscriptions.address['mgY65WSfEmsyYaYPQaXhmXMeBhwp4EcsQW'].length.should.equal(1);
      should.not.exist(squared.subscriptions.address['7d5169eBcGHF4BYC6DTffTyeCpWbrZnNgz']);
    });
  });
  ...
  describe('#_getDefaultConfig', function() {
    it('will generate config file from defaults', function() {
      var squared = new SquareService(baseConfig);
      var config = squared._getDefaultConfig();
      config.should.equal(defaultSquareConf);
    });
  });
  ...
  describe('#_loadSpawnConfiguration', function() {
    ...
    it('will parse a square.conf file', function() {
      var TestSquare = proxyquire('../../lib/services/squared', {
        fs: {
          readFileSync: readFileSync,
      ...
      var squared = new TestSquare(baseConfig);
      squared.options.spawn.datadir = '/tmp/.square';
      var node = {};
      squared._loadSpawnConfiguration(node);
      should.exist(squared.spawn.config);
      squared.spawn.config.should.deep.equal({
        addressindex: 1,
        checkblocks: 144,
        dbcache: 8192,
    ...
    it('will expand relative datadir to absolute path', function() {
      ...
      var config = {
        node: {
          network: squarecore.Networks.testnet,
          configPath: '/tmp/.squarecore/squarecore-node.json'
        },
        spawn: {
          datadir: './data',
          exec: 'testpath'
      ...
      var squared = new TestSquare(config);
      squared.options.spawn.datadir = './data';
      var node = {};
      squared._loadSpawnConfiguration(node);
      squared.options.spawn.datadir.should.equal('/tmp/.squarecore/data');
    });
    it('should throw an exception if txindex isn\'t enabled in the configuration', function() {
      var TestSquare = proxyquire('../../lib/services/squared', {
        fs: {
          readFileSync: sinon.stub().returns(fs.readFileSync(__dirname + '/../data/badsquare.conf')),
          existsSync: sinon.stub().returns(true),
        },
        mkdirp: {
          sync: sinon.stub()
      ...
      var squared = new TestSquare(baseConfig);
      (function() {
        squared._loadSpawnConfiguration({datadir: './test'});
      }).should.throw(squarecore.errors.InvalidState);
    });
  ...
  describe('#_checkConfigIndexes', function() {
    ...
    it('should warn the user if reindex is set to 1 in the square.conf file', function() {
      var squared = new SquareService(baseConfig);
      var config = {
        txindex: 1,
        addressindex: 1,
      ...
      var node = {};
      squared._checkConfigIndexes(config, node);
  ...
  describe('#_resetCaches', function() {
    it('will reset LRU caches', function() {
      ...
      for (var i = 0; i < 10; i++) {
        ...
        squared.transactionDetailedCache.set(keys[i], {});
        squared.utxosCache.set(keys[i], {});
        squared.txidsCache.set(keys[i], {});
        squared.balanceCache.set(keys[i], {});
        squared.summaryCache.set(keys[i], {});
      }
      squared._resetCaches();
      should.equal(squared.transactionDetailedCache.get(keys[0]), undefined);
      should.equal(squared.utxosCache.get(keys[0]), undefined);
      should.equal(squared.txidsCache.get(keys[0]), undefined);
      should.equal(squared.balanceCache.get(keys[0]), undefined);
      should.equal(squared.summaryCache.get(keys[0]), undefined);
    });
  });
  ...
  describe('#_tryAllClients', function() {
    it('will retry for each node client', function(done) {
      ...
      squared._tryAllClients(function(client, next) {
        client.getInfo(next);
      }, function(err) {
        if (err) {
          return done(err);
        }
        squared.nodes[0].client.getInfo.callCount.should.equal(1);
        squared.nodes[1].client.getInfo.callCount.should.equal(1);
        squared.nodes[2].client.getInfo.callCount.should.equal(1);
        done();
      });
    });
    it('will start using the current node index (round-robin)', function(done) {
      ...
      squared._tryAllClients(function(client, next) {
        ...
        squared.nodes[1].client.getInfo.callCount.should.equal(1);
        squared.nodes[2].client.getInfo.callCount.should.equal(1);
        squared.nodesIndex.should.equal(2);
        done();
      });
    });
  ...
  describe('#_initChain', function() {
    ...
    it('will set height and genesis buffer', function(done) {
      ...
      squared.getRawBlock = sinon.stub().callsArgWith(1, null, genesisBuffer);
      squared.nodes.push({
        client: {
      ...
      squared._initChain(function() {
        log.info.callCount.should.equal(1);
        squared.getRawBlock.callCount.should.equal(1);
        squared.getRawBlock.args[0][0].should.equal('genesishash');
        squared.height.should.equal(5000);
        squared.genesisBuffer.should.equal(genesisBuffer);
        done();
      });
    });
    it('it will handle error from getBestBlockHash', function(done) {
      ...
      squared._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('it will handle error from getRawBlock', function(done) {
      ...
      squared.getRawBlock = sinon.stub().callsArgWith(1, new Error('test'));
      squared._initChain(function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
  ...
  describe('#_getDefaultConf', function() {
    afterEach(function() {
      ...
      baseConfig.node.network = squarecore.Networks.testnet;
    });
    it('will get default rpc port for livenet', function() {
      var config = {
        node: {
          network: squarecore.Networks.livenet
        },
        spawn: {
          datadir: 'testdir',
          exec: 'testpath'
      ...
      var squared = new SquareService(config);
      squared._getDefaultConf().rpcport.should.equal(9998);
    });
    it('will get default rpc port for testnet', function() {
      ...
      var squared = new SquareService(config);
      squared._getDefaultConf().rpcport.should.equal(19998);
    });
  ...
  describe('#_getNetworkConfigPath', function() {
    ...
    it('will get default config path for livenet', function() {
      ...
      var squared = new SquareService(config);
      should.equal(squared._getNetworkConfigPath(), undefined);
    });
    it('will get default rpc port for testnet', function() {
      var config = {
        node: {
      ...
      var squared = new SquareService(config);
      squared._getNetworkConfigPath().should.equal('testnet3/square.conf');
    });
    it('will get default rpc port for regtest', function() {
      squarecore.Networks.enableRegtest();
      var config = {
      ...
      var squared = new SquareService(config);
      squared._getNetworkConfigPath().should.equal('regtest/square.conf');
    });
  });
  ...
  describe('#_getNetworkOption', function() {
    ...
    it('return --testnet for testnet', function() {
      var squared = new SquareService(baseConfig);
      squared.node.network = squarecore.Networks.testnet;
      squared._getNetworkOption().should.equal('--testnet');
    });
    it('return --regtest for testnet', function() {
      var squared = new SquareService(baseConfig);
      squared.node.network = squarecore.Networks.testnet;
      squarecore.Networks.enableRegtest();
      squared._getNetworkOption().should.equal('--regtest');
    });
    it('return undefined for livenet', function() {
      var squared = new SquareService(baseConfig);
      squared.node.network = squarecore.Networks.livenet;
      squarecore.Networks.enableRegtest();
      should.equal(squared._getNetworkOption(), undefined);
    });
  });
  ...
  describe('#_zmqBlockHandler', function() {
    it('will emit block', function(done) {
      ...
      squared._rapidProtectedUpdateTip = sinon.stub();
      squared.on('block', function(block) {
        block.should.equal(message);
        done();
      });
      squared._zmqBlockHandler(node, message);
    });
    ...
    it('will call function to update tip', function() {
      ...
      squared._rapidProtectedUpdateTip = sinon.stub();
      squared._zmqBlockHandler(node, message);
      squared._rapidProtectedUpdateTip.callCount.should.equal(1);
      squared._rapidProtectedUpdateTip.args[0][0].should.equal(node);
      squared._rapidProtectedUpdateTip.args[0][1].should.equal(message);
    });
    it('will emit to subscribers', function(done) {
      ...
      squared._rapidProtectedUpdateTip = sinon.stub();
      var emitter = new EventEmitter();
      squared.subscriptions.hashblock.push(emitter);
      emitter.on('squared/hashblock', function(blockHash) {
  ...
  describe('#_rapidProtectedUpdateTip', function() {
    it('will limit tip updates with rapid calls', function(done) {
      ...
      var count = 0;
      function repeat() {
        squared._rapidProtectedUpdateTip(node, message);
        count++;
        if (count < 50) {
          repeat();
        }
  ...
  describe('#_updateTip', function() {
    ...
    it('log and emit rpc error from get block', function(done) {
      var squared = new SquareService(baseConfig);
      squared.syncPercentage = sinon.stub();
      squared.on('error', function(err) {
      ...
      squared._updateTip(node, message);
    });
    it('emit synced if percentage is 100', function(done) {
      var squared = new SquareService(baseConfig);
      squared.syncPercentage = sinon.stub().callsArgWith(0, null, 100);
      squared.on('synced', function() {
        done();
      });
      var node = {
        client: {
    ...
    it('NOT emit synced if percentage is less than 100', function(done) {
      var squared = new SquareService(baseConfig);
      squared.syncPercentage = sinon.stub().callsArgWith(0, null, 99);
      squared.on('synced', function() {
    ...
    it('log and emit error from syncPercentage', function(done) {
      var squared = new SquareService(baseConfig);
      squared.syncPercentage = sinon.stub().callsArgWith(0, new Error('test'));
      squared.on('error', function(err) {
        log.error.callCount.should.equal(1);
        err.message.should.equal('test');
        done();
    ...
    it('reset caches and set height', function(done) {
      var squared = new SquareService(baseConfig);
      squared.syncPercentage = sinon.stub();
      squared._resetCaches = sinon.stub();
      squared.on('tip', function(height) {
        squared._resetCaches.callCount.should.equal(1);
        height.should.equal(10);
        squared.height.should.equal(10);
        done();
      });
      var node = {
        client: {
    ...
    it('will not call syncPercentage if node is stopping', function(done) {
      ...
      squared.on('tip', function() {
        squared.syncPercentage.callCount.should.equal(0);
        done();
      });
      squared._updateTip(node, message);
    });
  ...
  describe('#_getAddressesFromTransaction', function() {
    it('will get results using squarecore.Transaction', function() {
      var squared = new SquareService(baseConfig);
      var wif = 'XGLgPK8gbmzU7jcbw34Pj55AXV7SmG6carKuiwtu4WtvTjyTbpwX';
      var privkey = squarecore.PrivateKey.fromWIF(wif);
      var inputAddress = privkey.toAddress(squarecore.Networks.testnet);
      var outputAddress = squarecore.Address('8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi');
      var tx = squarecore.Transaction();
      tx.from({
        txid: '4a5e1e4baab89f3a32518a88c31bc87f618f76673e2cc77ab2127b7afdeda33b',
        outputIndex: 0,
        script: squarecore.Script(inputAddress),
        address: inputAddress.toString(),
        satoshis: 5000000000
      });
      tx.to(outputAddress, 5000000000);
      tx.sign(privkey);
      var addresses = squared._getAddressesFromTransaction(tx);
    ...
    it('will handle non-standard script types', function() {
      ...
      tx.addOutput(squarecore.Transaction.Output({
        script: squarecore.Script('OP_TRUE'),
        satoshis: 5000000000
      }));
      var addresses = squared._getAddressesFromTransaction(tx);
    ...
    it('will return unique values', function() {
      var squared = new SquareService(baseConfig);
      var tx = squarecore.Transaction();
      var address = squarecore.Address('8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi');
      tx.addOutput(squarecore.Transaction.Output({
        script: squarecore.Script(address),
        satoshis: 5000000000
      }));
      tx.addOutput(squarecore.Transaction.Output({
        script: squarecore.Script(address),
  ...
  describe('#_notifyAddressTxidSubscribers', function() {
    it('will emit event if matching addresses', function(done) {
      ...
      squared._getAddressesFromTransaction = sinon.stub().returns([address]);
      var emitter = new EventEmitter();
      squared.subscriptions.address[address] = [emitter];
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var transaction = {};
      emitter.on('squared/addresstxid', function(data) {
        data.address.should.equal(address);
        data.txid.should.equal(txid);
        done();
      });
      sinon.spy(emitter, 'emit');
      squared._notifyAddressTxidSubscribers(txid, transaction);
  ...
  describe('#_zmqTransactionHandler', function() {
    it('will emit to subscribers', function(done) {
      ...
      var emitter = new EventEmitter();
      squared.subscriptions.rawtransaction.push(emitter);
      emitter.on('squared/rawtransaction', function(hex) {
        hex.should.be.a('string');
        hex.should.equal(expectedBuffer.toString('hex'));
        done();
      });
      var node = {};
      squared._zmqTransactionHandler(node, expectedBuffer);
    });
    it('will NOT emit to subscribers more than once for the same tx', function(done) {
      ...
      var emitter = new EventEmitter();
      squared.subscriptions.rawtransaction.push(emitter);
      emitter.on('squared/rawtransaction', function() {
        done();
      });
      var node = {};
      squared._zmqTransactionHandler(node, expectedBuffer);
  ...
  describe('#_zmqTransactionLockHandler', function() {
    it('will emit to subscribers', function(done) {
      ...
      var expectedBuffer = new Buffer(txhex, 'hex');
      var emitter = new EventEmitter();
      squared.subscriptions.transactionlock.push(emitter);
      emitter.on('squared/transactionlock', function(hex) {
        hex.should.be.a('string');
        hex.should.equal(expectedBuffer.toString('hex'));
        done();
      });
      var node = {};
      squared._zmqTransactionLockHandler(node, expectedBuffer);
    });
    it('will NOT emit to subscribers more than once for the same tx', function(done) {
      var squared = new SquareService(baseConfig);
      var expectedBuffer = new Buffer(txhex, 'hex');
      var emitter = new EventEmitter();
      squared.subscriptions.transactionlock.push(emitter);
      emitter.on('squared/transactionlock', function() {
        done();
      });
      var node = {};
      squared._zmqTransactionLockHandler(node, expectedBuffer);
    ...
    it('will emit "tx" event', function(done) {
      var squared = new SquareService(baseConfig);
      var expectedBuffer = new Buffer(txhex, 'hex');
      squared.on('txlock', function(buffer) {
        buffer.should.be.instanceof(Buffer);
        buffer.toString('hex').should.equal(expectedBuffer.toString('hex'));
        done();
      });
  ...
  describe('#_checkSyncedAndSubscribeZmqEvents', function() {
    ...
    it('log errors, update tip and subscribe to zmq events', function(done) {
      var squared = new SquareService(baseConfig);
      squared._updateTip = sinon.stub();
      squared._subscribeZmqEvents = sinon.stub();
      var blockEvents = 0;
      squared.on('block', function() {
        blockEvents++;
      });
      var getBestBlockHash = sinon.stub().callsArgWith(0, null, {
      ...
      squared._checkSyncedAndSubscribeZmqEvents(node);
      setTimeout(function() {
        log.error.callCount.should.equal(2);
        blockEvents.should.equal(11);
        squared._updateTip.callCount.should.equal(11);
        squared._subscribeZmqEvents.callCount.should.equal(1);
        done();
      }, 200);
    });
  ...
  describe('#_subscribeZmqEvents', function() {
    it('will call subscribe on zmq socket', function() {
      ...
      squared._subscribeZmqEvents(node);
    ...
    it('will call relevant handler for rawtx topics', function(done) {
      var squared = new SquareService(baseConfig);
      squared._zmqTransactionHandler = sinon.stub();
      var node = {
      ...
      node.zmqSubSocket.on('message', function() {
        squared._zmqTransactionHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will call relevant handler for hashblock topics', function(done) {
      var squared = new SquareService(baseConfig);
      squared._zmqBlockHandler = sinon.stub();
      var node = {
        zmqSubSocket: new EventEmitter()
      };
      node.zmqSubSocket.subscribe = sinon.stub();
      squared._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        squared._zmqBlockHandler.callCount.should.equal(1);
        done();
      });
    ...
    it('will ignore unknown topic types', function(done) {
      ...
      squared._subscribeZmqEvents(node);
      node.zmqSubSocket.on('message', function() {
        squared._zmqBlockHandler.callCount.should.equal(0);
        squared._zmqTransactionHandler.callCount.should.equal(0);
        done();
      });
  ...
  describe('#_initZmqSubSocket', function() {
    it('will setup zmq socket', function() {
      ...
      var squared = new SquareService(baseConfig);
      var node = {};
      squared._initZmqSubSocket(node, 'url');
  ...
  describe('#_checkReindex', function() {
    ...
    it('give error from client getblockchaininfo', function(done) {
      ...
      squared._checkReindex(node, function(err) {
        should.exist(err);
    ...
    it('will wait until sync is 100 percent', function(done) {
      ...
      squared._checkReindex(node, function() {
  ...
  describe('#_loadTipFromNode', function() {
    ...
    it('will give rpc from client getbestblockhash', function(done) {
      ...
      squared._loadTipFromNode(node, function(err) {
        err.should.be.instanceof(Error);
    ...
    it('will set height and emit tip', function(done) {
      ...
      squared.on('tip', function(height) {
        ...
        squared.height.should.equal(100);
        done();
      });
      squared._loadTipFromNode(node, function(err) {
        if (err) {
  ...
  describe('#_stopSpawnedProcess', function() {
    ...
    it('it will kill process and resume', function(done) {
      ...
      var TestSquareService = proxyquire('../../lib/services/squared', {
        fs: {
          readFile: readFile
        }
      });
      var squared = new TestSquareService(baseConfig);
      squared.spawnStopTime = 1;
      squared._process = {};
      squared._process.kill = sinon.stub();
      squared._stopSpawnedSquare(function(err) {
        if (err) {
          return done(err);
        }
        squared._process.kill.callCount.should.equal(1);
        log.warn.callCount.should.equal(1);
        done();
      });
    });
    it('it will attempt to kill process and resume', function(done) {
      ...
      error2.code = 'ESRCH';
      squared._process.kill = sinon.stub().throws(error2);
      squared._stopSpawnedSquare(function(err) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_spawnChildProcess', function() {
    ...
    it('will give error from spawn config', function(done) {
      var squared = new SquareService(baseConfig);
      squared._loadSpawnConfiguration = sinon.stub();
      squared._loadSpawnConfiguration = sinon.stub().throws(new Error('test'));
      squared._spawnChildProcess(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    });
    it('will give error from stopSpawnedSquare', function() {
      var squared = new SquareService(baseConfig);
      squared._loadSpawnConfiguration = sinon.stub();
      squared._stopSpawnedSquare = sinon.stub().callsArgWith(0, new Error('test'));
      squared._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
      });
    ...
    it('will exit spawn if shutdown', function() {
      ...
      var squared = new TestSquareService(config);
      squared.spawn = {};
      squared._loadSpawnConfiguration = sinon.stub();
      squared._stopSpawnedSquare = sinon.stub().callsArgWith(0, null);
      squared.node.stopping = true;
      squared._spawnChildProcess(function(err) {
        err.should.be.instanceOf(Error);
    ...
    it('will include network with spawn command and init zmq/rpc on node', function(done) {
      ...
      squared._loadSpawnConfiguration = sinon.stub();
      squared.spawn = {};
      squared.spawn.exec = 'testexec';
      squared.spawn.configPath = 'testdir/square.conf';
      squared.spawn.datadir = 'testdir';
      squared.spawn.config = {};
      squared.spawn.config.rpcport = 20001;
      squared.spawn.config.rpcuser = 'square';
      squared.spawn.config.rpcpassword = 'password';
      squared.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
      squared.spawn.config.zmqpubrawtxlock = 'tcp://127.0.0.1:30001';

      squared._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      squared._initZmqSubSocket = sinon.stub();
      squared._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      squared._checkReindex = sinon.stub().callsArgWith(1, null);
      squared._spawnChildProcess(function(err, node) {
        should.not.exist(err);
        spawn.callCount.should.equal(1);
        spawn.args[0][0].should.equal('testexec');
        spawn.args[0][1].should.deep.equal([
          '--conf=testdir/square.conf',
          '--datadir=testdir',
          '--testnet'
        ]);
        ...
        squared._loadTipFromNode.callCount.should.equal(1);
        squared._initZmqSubSocket.callCount.should.equal(1);
        should.exist(squared._initZmqSubSocket.args[0][0].client);
        squared._initZmqSubSocket.args[0][1].should.equal('tcp://127.0.0.1:30001');
        squared._checkSyncedAndSubscribeZmqEvents.callCount.should.equal(1);
        should.exist(squared._checkSyncedAndSubscribeZmqEvents.args[0][0].client);
        should.exist(node);
        should.exist(node.client);
        done();
      });
    });
    it('will respawn squared spawned process', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestSquareService = proxyquire('../../lib/services/squared', {
        fs: {
      ...
      squared._loadSpawnConfiguration = sinon.stub();
      squared.spawn = {};
      squared.spawn.exec = 'squared';
      squared.spawn.datadir = '/tmp/square';
      squared.spawn.configPath = '/tmp/square/square.conf';
      squared.spawn.config = {};
      squared.spawnRestartTime = 1;
      squared._loadTipFromNode = sinon.stub().callsArg(1);
      squared._initZmqSubSocket = sinon.stub();
      squared._checkReindex = sinon.stub().callsArg(1);
      squared._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      squared._stopSpawnedSquare = sinon.stub().callsArg(0);
      sinon.spy(squared, '_spawnChildProcess');
      squared._spawnChildProcess(function(err) {
        if (err) {
          return done(err);
        }
        process.once('exit', function() {
          setTimeout(function() {
            squared._spawnChildProcess.callCount.should.equal(2);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will emit error during respawn', function(done) {
      ...
      squared._spawnChildProcess(function(err) {
        ...
        squared._spawnChildProcess = sinon.stub().callsArgWith(0, new Error('test'));
        squared.on('error', function(err) {
          err.should.be.instanceOf(Error);
          err.message.should.equal('test');
          done();
    ...
    it('will NOT respawn squared spawned process if shutting down', function(done) {
      var process = new EventEmitter();
      var spawn = sinon.stub().returns(process);
      var TestSquareService = proxyquire('../../lib/services/squared', {
        fs: {
      ...
      squared._spawnChildProcess(function(err) {
        ...
        process.once('exit', function() {
          setTimeout(function() {
            squared._spawnChildProcess.callCount.should.equal(1);
            done();
          }, 5);
        });
        process.emit('exit', 1);
    ...
    it('will give error after 60 retries', function(done) {
      ...
      squared.spawn.config.zmqpubrawtx = 'tcp://127.0.0.1:30001';
      squared.spawn.config.zmqpubrawtxlock = 'tcp://127.0.0.1:30001';
      squared._loadTipFromNode = sinon.stub().callsArgWith(1, new Error('test'));
      squared._spawnChildProcess(function(err) {
        squared._loadTipFromNode.callCount.should.equal(60);
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give error from check reindex', function(done) {
      ...
      squared._initZmqSubSocket = sinon.stub();
      squared._checkSyncedAndSubscribeZmqEvents = sinon.stub();
      squared._checkReindex = sinon.stub().callsArgWith(1, new Error('test'));
  ...
  describe('#_connectProcess', function() {
    it('will give error if connecting while shutting down', function(done) {
      ...
      var squared = new SquareService(config);
      squared.node.stopping = true;
      squared.startRetryInterval = 100;
      squared._loadTipFromNode = sinon.stub();
      squared._connectProcess({}, function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/Stopping while trying to connect/);
        squared._loadTipFromNode.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from loadTipFromNode after 60 retries', function(done) {
      ...
      squared.startRetryInterval = 1;
      var config = {};
      squared._connectProcess(config, function(err) {
        err.should.be.instanceof(Error);
        squared._loadTipFromNode.callCount.should.equal(60);
        done();
      });
    ...
    it('will init zmq/rpc on node', function(done) {
      ...
      squared._loadTipFromNode = sinon.stub().callsArgWith(1, null);
      var config = {};
      squared._connectProcess(config, function(err, node) {
        should.not.exist(err);
        squared._loadTipFromNode.callCount.should.equal(1);
        squared._initZmqSubSocket.callCount.should.equal(1);
        squared._loadTipFromNode.callCount.should.equal(1);
  ...
  describe('#start', function() {
    ...
    it('will give error if "spawn" and "connect" are both not configured', function(done) {
      var squared = new SquareService(baseConfig);
      squared.options = {};
      squared.start(function(err) {
        err.should.be.instanceof(Error);
        err.message.should.match(/Square configuration options/);
      });
      done();
    });
    ...
    it('will give error from connectProcess', function(done) {
      var squared = new SquareService(baseConfig);
      squared._connectProcess = sinon.stub().callsArgWith(1, new Error('test'));
      squared.options = {
        connect: [
          {}
        ]
      ...
      squared.start(function(err) {
        squared._connectProcess.callCount.should.equal(1);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will push node from spawnChildProcess', function(done) {
      var squared = new SquareService(baseConfig);
      var node = {};
      squared._initChain = sinon.stub().callsArg(0);
      squared._spawnChildProcess = sinon.stub().callsArgWith(0, null, node);
      squared.options = {
        spawn: {}
      };
      squared.start(function(err) {
        should.not.exist(err);
        squared.nodes.length.should.equal(1);
        done();
      });
    });
    it('will push node from connectProcess', function(done) {
      ...
      squared._initChain = sinon.stub().callsArg(0);
      var nodes = [{}];
      squared._connectProcess = sinon.stub().callsArgWith(1, null, nodes);
      squared.options = {
        connect: [
          {}
        ]
  ...
  describe('#isSynced', function() {
    ...
    it('will give "true" if percentage is 100.00', function(done) {
      var squared = new SquareService(baseConfig);
      squared.syncPercentage = sinon.stub().callsArgWith(0, null, 100.00);
      squared.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "true" if percentage is 99.98', function(done) {
      var squared = new SquareService(baseConfig);
      squared.syncPercentage = sinon.stub().callsArgWith(0, null, 99.98);
      squared.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 99.49', function(done) {
      var squared = new SquareService(baseConfig);
      squared.syncPercentage = sinon.stub().callsArgWith(0, null, 99.49);
      squared.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
    ...
    it('will give "false" if percentage is 1', function(done) {
      var squared = new SquareService(baseConfig);
      squared.syncPercentage = sinon.stub().callsArgWith(0, null, 1);
      squared.isSynced(function(err, synced) {
        if (err) {
          return done(err);
        }
  ...
  describe('#syncPercentage', function() {
    it('will give rpc error', function(done) {
      ...
      squared.syncPercentage(function(err) {
        should.exist(err);
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      squared.syncPercentage(function(err, percentage) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_normalizeAddressArg', function() {
    it('will turn single address into array', function() {
      var squared = new SquareService(baseConfig);
      var args = squared._normalizeAddressArg('address');
      args.should.deep.equal(['address']);
    });
    it('will keep an array as an array', function() {
      var squared = new SquareService(baseConfig);
      var args = squared._normalizeAddressArg(['address', 'address']);
  ...
  describe('#getAddressBalance', function() {
    it('will give rpc error', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      var options = {};
      squared.getAddressBalance(address, options, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    it('will give balance', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      var options = {};
      squared.getAddressBalance(address, options, function(err, data) {
        if (err) {
          return done(err);
        }
        data.balance.should.equal(10000);
        data.received.should.equal(100000);
        squared.getAddressBalance(address, options, function(err, data2) {
          if (err) {
            return done(err);
          }
  ...
  describe('#getAddressUnspentOutputs', function() {
    it('will give rpc error', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      squared.getAddressUnspentOutputs(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give results from client getaddressutxos', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      squared.getAddressUnspentOutputs(address, options, function(err, utxos) {
        if (err) {
          return done(err);
        }
  ...
  describe('#_getBalanceFromMempool', function() {
    it('will sum satoshis', function() {
      ...
      var sum = squared._getBalanceFromMempool(deltas);
  ...
  describe('#_getTxidsFromMempool', function() {
    it('will filter to txids', function() {
      ...
      var txids = squared._getTxidsFromMempool(deltas);
  ...
  describe('#_getHeightRangeQuery', function() {
    it('will detect range query', function() {
      ...
      var rangeQuery = squared._getHeightRangeQuery(options);
    ...
    it('will get range properties', function() {
      ...
      var clone = {};
      squared._getHeightRangeQuery(options, clone);
    ...
    it('will throw error with invalid range', function() {
      ...
      (function() {
        squared._getHeightRangeQuery(options);
  ...
  describe('#getAddressTxids', function() {
    it('will give error from _getHeightRangeQuery', function(done) {
      var squared = new SquareService(baseConfig);
      squared._getHeightRangeQuery = sinon.stub().throws(new Error('test'));
      squared.getAddressTxids('address', {}, function(err) {
        err.should.be.instanceOf(Error);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give rpc error from mempool query', function() {
      ...
      var options = {};
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      squared.getAddressTxids(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
      });
    });
    ...
    it('will get txid results', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      squared.getAddressTxids(address, options, function(err, txids) {
        if (err) {
          return done(err);
        }
    ...
    it('will get txid results from cache and live mempool', function(done) {
      ...
      var address = 'XnQuJpAgEDNtRwoXWLfuEs69cMgCYS8rgs';
      squared.getAddressTxids(address, {queryMempool: false}, function(err, txids) {
        if (err) {
          return done(err);
        }
        getAddressTxids.callCount.should.equal(1);
        ...
        squared.getAddressTxids(address, {queryMempool: true}, function(err, txids) {
          if (err) {
            return done(err);
          }
          getAddressTxids.callCount.should.equal(1);
  ...
  describe('#_getConfirmationDetail', function() {
    ...
    it('should get 0 confirmation', function() {
      ...
      var squared = new SquareService(baseConfig);
      squared.height = 10;
      var confirmations = squared._getConfirmationsDetail(tx);
  ...
  describe('#_getAddressDetailsForInput', function() {
    it('will return if missing an address', function() {
      var squared = new SquareService(baseConfig);
      var result = {};
      squared._getAddressDetailsForInput({}, 0, result, []);
    ...
    it('will only add address if it matches', function() {
      var squared = new SquareService(baseConfig);
      var result = {};
      squared._getAddressDetailsForInput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForOutput', function() {
    it('will return if missing an address', function() {
      var squared = new SquareService(baseConfig);
      var result = {};
      squared._getAddressDetailsForOutput({}, 0, result, []);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
    });
    it('will only add address if it matches', function() {
      var squared = new SquareService(baseConfig);
      var result = {};
      squared._getAddressDetailsForOutput({
        address: 'address1'
      }, 0, result, ['address2']);
      should.not.exist(result.addresses);
      should.not.exist(result.satoshis);
  ...
  describe('#_getAddressDetailsForTransaction', function() {
    it('will calculate details for the transaction', function(done) {
      ...
      var details = squared._getAddressDetailsForTransaction(tx, addresses);
  ...
  describe('#_getAddressDetailedTransaction', function() {
    it('will get detailed transaction info', function(done) {
      ...
      var squared = new SquareService(baseConfig);
      squared.getDetailedTransaction = sinon.stub().callsArgWith(1, null, tx);
      squared.height = 300;
      var addresses = {};
      squared._getAddressDetailsForTransaction = sinon.stub().returns({
        addresses: addresses,
        satoshis: 1000,
      });
      squared._getAddressDetailedTransaction(txid, {}, function(err, details) {
        if (err) {
          return done(err);
        }
    ...
    it('give error from getDetailedTransaction', function(done) {
      var txid = '46f24e0c274fc07708b781963576c4c5d5625d926dbb0a17fa865dcd9fe58ea0';
      var squared = new SquareService(baseConfig);
      squared.getDetailedTransaction = sinon.stub().callsArgWith(1, new Error('test'));
      squared._getAddressDetailedTransaction(txid, {}, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
  ...
  describe('#_getAddressStrings', function() {
    it('will get address strings from squarecore addresses', function() {
      var addresses = [
        squarecore.Address('XjxDQFjTNEP9dcrJhBLvy5i1Dobz4x1LJN'),
        squarecore.Address('7d5169eBcGHF4BYC6DTffTyeCpWbrZnNgz'),
      ];
      var squared = new SquareService(baseConfig);
      var strings = squared._getAddressStrings(addresses);
    ...
    it('will give error with unknown', function() {
      ...
      var squared = new SquareService(baseConfig);
      (function() {
        squared._getAddressStrings(addresses);
  ...
  describe('#getAddressHistory', function() {
    ...
    it('will give error with "from" and "to" range that exceeds max size', function(done) {
      var squared = new SquareService(baseConfig);
      squared.getAddressHistory(address, {from: 0, to: 51}, function(err) {
        should.exist(err);
    ...
    it('will give error with "from" and "to" order is reversed', function(done) {
      var squared = new SquareService(baseConfig);
      squared.getAddressTxids = sinon.stub().callsArgWith(2, null, []);
      squared.getAddressHistory(address, {from: 51, to: 0}, function(err) {
        should.exist(err);
        err.message.match(/^\"from/);
        done();
      });
    ...
    it('will give error from _getAddressDetailedTransaction', function(done) {
      var squared = new SquareService(baseConfig);
      squared.getAddressTxids = sinon.stub().callsArgWith(2, null, ['txid']);
      squared._getAddressDetailedTransaction = sinon.stub().callsArgWith(2, new Error('test'));
      squared.getAddressHistory(address, {}, function(err) {
        should.exist(err);
        err.message.should.equal('test');
        done();
      });
    ...
    it('will give an error if length of addresses is too long', function(done) {
      ...
      var squared = new SquareService(baseConfig);
      squared.maxAddressesQuery = 100;
      squared.getAddressHistory(addresses, {}, function(err) {
        should.exist(err);
    ...
    it('give error from getAddressTxids', function(done) {
      var squared = new SquareService(baseConfig);
      squared.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      squared.getAddressHistory('address', {}, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
    ...
    it('will paginate', function(done) {
      var squared = new SquareService(baseConfig);
      squared._getAddressDetailedTransaction = function(txid, options, callback) {
        callback(null, txid);
      };
      var txids = ['one', 'two', 'three', 'four'];
      squared.getAddressTxids = sinon.stub().callsArgWith(2, null, txids);
      squared.getAddressHistory('address', {from: 1, to: 3}, function(err, data) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getAddressSummary', function() {
    ...
    it('will handle error from getAddressTxids', function(done) {
      ...
      squared.getAddressTxids = sinon.stub().callsArgWith(2, new Error('test'));
      squared.getAddressBalance = sinon.stub().callsArgWith(2, null, {});
      var address = '';
      var options = {};
      squared.getAddressSummary(address, options, function(err) {
        should.exist(err);
        err.should.be.instanceof(Error);
        err.message.should.equal('test');
        done();
    ...
    it('will handle error from getAddressBalance', function(done) {
      ...
      squared.getAddressTxids = sinon.stub().callsArgWith(2, null, {});
      squared.getAddressBalance = sinon.stub().callsArgWith(2, new Error('test'), {});
      var address = '';
      var options = {};
      squared.getAddressSummary(address, options, function(err) {
        should.exist(err);
    ...
    it('should set all properties', function(done) {
      ...
      sinon.spy(squared, '_paginateTxids');
      squared.getAddressTxids = sinon.stub().callsArgWith(2, null, [txid1, txid2, txid3]);
      squared.getAddressBalance = sinon.stub().callsArgWith(2, null, {
        received: 30 * 1e8,
        balance: 20 * 1e8
      });
      ...
      var options = {};
      squared.getAddressSummary(address, options, function(err, summary) {
        squared._paginateTxids.callCount.should.equal(1);
        squared._paginateTxids.args[0][1].should.equal(0);
        squared._paginateTxids.args[0][2].should.equal(1000);
    ...
    it('will get from cache with noTxList', function(done) {
      ...
      squared.getAddressSummary(address, options, function(err, summary) {
        ...
        squared.getAddressTxids.callCount.should.equal(1);
        squared.getAddressBalance.callCount.should.equal(1);
        squared.getAddressSummary(address, options, function(err, summary) {
          checkSummary(summary);
          squared.getAddressTxids.callCount.should.equal(1);
          squared.getAddressBalance.callCount.should.equal(1);
    ...
    it('will skip querying the mempool with queryMempool set to false', function(done) {
      ...
      squared.getAddressSummary(address, options, function() {
        getAddressMempool.callCount.should.equal(0);
        done();
      });
    });
    it('will give error from _paginateTxids', function(done) {
      ...
      squared._paginateTxids = sinon.stub().throws(new Error('test'));
      var address = '7oK6xjGeVK5YCT5dpqzNXGUag1bQadPAyT';
      var options = {
        queryMempool: false
      };
  ...
  describe('#getRawBlock', function() {
    ...
    it('will give rcp error from client getblockhash', function(done) {
      ...
      squared.getRawBlock(10, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will give rcp error from client getblock', function(done) {
      ...
      squared.getRawBlock(blockhash, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes for getblock', function(done) {
      ...
      squared.getRawBlock(blockhash, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
    ...
    it('will get block by height', function(done) {
      ...
      squared.getRawBlock(0, function(err, buffer) {
        if (err) {
          return done(err);
        }
        buffer.should.be.instanceof(Buffer);
  ...
  describe('#getBlock', function() {
    ...
    it('will give an rpc error from client getblock', function(done) {
      ...
      squared.getBlock(0, function(err) {
        err.should.be.instanceof(Error);
        done();
      });
    });
    ...
    it('will getblock as squarecore object from height', function(done) {
      var squared = new SquareService(baseConfig);
      var getBlock = sinon.stub().callsArgWith(2, null, {
        result: blockhex
      });
      ...
      squared.getBlock(0, function(err, block) {
        should.not.exist(err);
        ...
        block.should.be.instanceof(squarecore.Block);
        done();
      });
    });
    it('will getblock as squarecore object', function(done) {
      var squared = new SquareService(baseConfig);
      var getBlock = sinon.stub().callsArgWith(2, null, {
        result: blockhex
      });
      ...
      squared.getBlock('00000000050a6d07f583beba2d803296eb1e9d4980c4a20f206c584e89a4f02b', function(err, block) {
        should.not.exist(err);
    ...
    it('will get block from cache', function(done) {
      ...
      squared.getBlock(hash, function(err, block) {
        should.not.exist(err);
        getBlockHash.callCount.should.equal(0);
        getBlock.callCount.should.equal(1);
        block.should.be.instanceof(squarecore.Block);
  ...
  describe('#getBlockHashesByTimestamp', function() {
    it('should give an rpc error', function(done) {
      ...
      squared.getBlockHashesByTimestamp(1441911000, 1441914000, function(err, hashes) {
        should.exist(err);
    ...
    it('should get the correct block hashes', function(done) {
      ...
      squared.getBlockHashesByTimestamp(1441914000, 1441911000, function(err, hashes) {
        should.not.exist(err);
  ...
  describe('#getBlockHeader', function() {
    ...
    it('will give error from getBlockHash', function() {
      ...
      squared.getBlockHeader(10, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockheader', function() {
      ...
      squared.getBlockHeader(blockhash, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('it will give rpc error from client getblockhash', function() {
      ...
      squared.getBlockHeader(0, function(err) {
        err.should.be.instanceof(Error);
      });
    });
    it('will give result from client getblockheader (from height)', function() {
      ...
      squared.getBlockHeader(0, function(err, blockHeader) {
        should.not.exist(err);
    ...
    it('will give result from client getblockheader (from hash)', function() {
      ...
      squared.getBlockHeader(blockhash, function(err, blockHeader) {
        should.not.exist(err);
        getBlockHash.callCount.should.equal(0);
        blockHeader.should.deep.equal(result);
      });
  ...
  describe('#_maybeGetBlockHash', function() {
    it('will not get block hash with an address', function(done) {
      ...
      squared._maybeGetBlockHash('8oUSpiq5REeEKAzS1qSXoJbZ9TRfH1L6mi', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will not get block hash with non zero-nine numeric string', function(done) {
      ...
      squared._maybeGetBlockHash('109a', function(err, hash) {
        if (err) {
          return done(err);
        }
        getBlockHash.callCount.should.equal(0);
    ...
    it('will get the block hash if argument is a number', function(done) {
      ...
      squared._maybeGetBlockHash(10, function(err, hash) {
        if (err) {
          return done(err);
        }
  ...
  describe('#getBlockOverview', function() {
    ...
    it('will handle error from maybeGetBlockHash', function(done) {
      var squared = new SquareService(baseConfig);
      squared._maybeGetBlockHash = sinon.stub().callsArgWith(1, new Error('test'));
      squared.getBlockOverview(blockhash, function(err) {
        err.should.be.instanceOf(Error);
        done();
      });
    });
    ...
    it('will give expected result', function(done) {
      ...
      squared.getBlockOverview(blockhash, function(err, blockOverview) {
        if (err) {
          return done(err);
        }
  ...
  describe('#estimateFee', function() {
    it('will give rpc error', function(done) {
      ...
      squared.estimateFee(1, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client estimateFee and give result', function(done) {
      ...
      squared.estimateFee(1, function(err, feesPerKb) {
        if (err) {
          return done(err);
        }
  ...
  describe('#sendTransaction', function(done) {
    var tx = squarecore.Transaction(txhex);
    it('will give rpc error', function() {
      ...
      squared.sendTransaction(txhex, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
      });
    });
    it('will send to client and get hash', function() {
      ...
      squared.sendTransaction(txhex, function(err, hash) {
        if (err) {
          return done(err);
        }
    ...
    it('will send to client with absurd fees and get hash', function() {
      ...
      squared.sendTransaction(txhex, {allowAbsurdFees: true}, function(err, hash) {
        if (err) {
          return done(err);
        }
        hash.should.equal(tx.hash);
    ...
    it('missing callback will throw error', function() {
      ...
      var transaction = squarecore.Transaction();
      (function() {
        squared.sendTransaction(transaction);
      }).should.throw(Error);
    });
  });
  ...
  describe('#getRawTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      squared.getRawTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      squared.getRawTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
  ...
  describe('#getTransaction', function() {
    it('will give rpc error', function(done) {
      ...
      squared.getTransaction('txid', function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will try all nodes', function(done) {
      ...
      squared.getTransaction('txid', function(err, tx) {
        if (err) {
          return done(err);
        }
        should.exist(tx);
        tx.should.be.an.instanceof(squarecore.Transaction);
        done();
      });
    });
    it('will get from cache', function(done) {
  ...
  describe('#getDetailedTransaction', function() {
    ...
    it('should give a transaction with height and timestamp', function(done) {
      ...
      squared.getDetailedTransaction(txid, function(err) {
        should.exist(err);
        err.should.be.instanceof(errors.RPCError);
        done();
      });
    ...
    it('should give a transaction with all properties', function(done) {
      ...
      squared.getDetailedTransaction(txid, function(err, tx) {
        if (err) {
          return done(err);
        }
        checkTx(tx);
  ...
  describe('#getBestBlockHash', function() {
    it('will give rpc error', function(done) {
      ...
      squared.getBestBlockHash(function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client getInfo and give result', function(done) {
      ...
      squared.getBestBlockHash(function(err, hash) {
        if (err) {
          return done(err);
        }
        should.exist(hash);
  ...
  describe('#getSpentInfo', function() {
    it('will give rpc error', function(done) {
      ...
      squared.getSpentInfo({}, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will empty object when not found', function(done) {
      ...
      squared.getSpentInfo({}, function(err, info) {
        should.not.exist(err);
  ...
  describe('#getInfo', function() {
    ...
    it('will call client getInfo and give result', function(done) {
      var squared = new SquareService(baseConfig);
      squared.node.getNetworkName = sinon.stub().returns('testnet');
      ...
      squared.getInfo(function(err, info) {
        if (err) {
          return done(err);
        }
        should.exist(info);
  ...
  describe('#govObject', function() {
    it('will call client gobject list and give result', function(done) {
        ...
        squared.govObjectList({type: 1}, function(err, result) {
            if (err) {
                return done(err);
            }
            should.exist(result);
    ...
    it('will call client gobject get and give result', function(done) {
      ...
      var gobject = sinon.stub().callsArgWith(2, null, {
        result: {
          "DataHex": "",
          "DataString": "[[\"proposal\",{\"end_epoch\":1483855193,\"name\":\"7ea9acff5ae83d8c9e217d30a2b4d10ef8f171f8\",\"payment_address\":\"yZ7DYkDHCHfH1dw7rKdYaKcVykZmunbqNL\",\"payment_amount\":84,\"start_epoch\":1483765282,\"type\":1,\"url\":\"https://www.square.org\"}]]",
          "Hash": "4ef24027c631c43035aa4cf5c672e1298311decd9cffbd16731f454c9c0d6d00",
      ...
      squared.govObjectHash('4ef24027c631c43035aa4cf5c672e1298311decd9cffbd16731f454c9c0d6d00', function(err, result) {
        if (err) {
          return done(err);
        }
        ...
        should.equal(DataObject.url, 'https://www.square.org');
        done();
      });
    });
  ...
  describe('#generateBlock', function() {
    it('will give rpc error', function(done) {
      ...
      squared.generateBlock(10, function(err) {
        should.exist(err);
        err.should.be.an.instanceof(errors.RPCError);
        done();
      });
    ...
    it('will call client generate and give result', function(done) {
      ...
      squared.generateBlock(10, function(err, hashes) {
        if (err) {
          return done(err);
        }
  ...
  describe('#stop', function() {
    ...
    it('will exit spawned process', function(done) {
      var squared = new SquareService(baseConfig);
      squared.spawn = {};
      squared.spawn.process = new EventEmitter();
      squared.spawn.process.kill = sinon.stub();
      squared.stop(done);
      squared.spawn.process.kill.callCount.should.equal(1);
      squared.spawn.process.kill.args[0][0].should.equal('SIGINT');
      squared.spawn.process.emit('exit', 0);
    });
    it('will give error with non-zero exit status code', function(done) {
      ...
      squared.spawn.process.kill.callCount.should.equal(1);
      squared.spawn.process.kill.args[0][0].should.equal('SIGINT');
      squared.spawn.process.emit('exit', 1);
    });
    it('will stop after timeout', function(done) {
      var squared = new SquareService(baseConfig);
      squared.shutdownTimeout = 300;
      squared.spawn = {};
      squared.spawn.process = new EventEmitter();
      squared.spawn.process.kill = sinon.stub();
      squared.stop(function(err) {
